...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if Player.Functions.RemoveItem(itemData.name, itemAmount, itemData.slot) then
		...
		TriggerEvent("qb-log:server:CreateLog", "drop", "Suelo", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) Tiro este item **"..itemData.name.."**, cantidad: **" .. itemAmount .. "**")
		...
		TriggerClientEvent("QBCore:Notify", source, "No tienes este artículo", "error")
		return
	end
end
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		...
		TriggerClientEvent('QBCore:Notify', src, '¡El inventario está bloqueado!', 'error')
	end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qb-log:server:CreateLog", "robbing", "Robo", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) robo de item: **"..itemInfo["name"].."**, cantidad: **" .. toAmount .. "** con el nombre **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("qb-log:server:CreateLog", "robbing", "Robo", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*)  robo de item: **"..itemInfo["name"].."**, cantidad: **" .. fromAmount .. "** a la persona **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qb-log:server:CreateLog", "trunk", "Maletero", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qb-log:server:CreateLog", "glovebox", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("qb-log:server:CreateLog", "stash", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
				...
				if IsItemValid then
					...
					if toItemData ~= nil then
						...
						if toItemData.name ~= fromItemData.name then
							...
							Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("qb-log:server:CreateLog", "traphouse", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
					...
					TriggerClientEvent('QBCore:Notify', src, "No puedes vender esto.", 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData ~= nil then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("qb-log:server:CreateLog", "drop", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			TriggerClientEvent("QBCore:Notify", src, "No tienes este item", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "robbing", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** from player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
			...
			TriggerClientEvent("QBCore:Notify", src, "Este objeto no existe.", "error")
		end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "trunk", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "trunk", "Objeto apilado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "trunk", "Artículo recibido", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "glovebox", "Objeto apilado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "glovebox", "Artículo recibido", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "stash", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "stash", "Objeto apilado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "stash", "Artículo recibido", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "stash", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "stash", "Objeto apilado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "stash", "Artículo recibido", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['qb-traphouse']:GetInventoryData(traphouseId, toSlot)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " comprado!", "success")
					TriggerEvent("qb-log:server:CreateLog", "dealers", "Artículo del distribuidor comprado", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					TriggerClientEvent('QBCore:Notify', src, "No tienes dinero", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " comprado!", "success")
				TriggerEvent("qb-log:server:CreateLog", "shops", "Artículo comprado", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "No tienes los artículos correctos", "error")
		end
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "drop", "artículo intercambiado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) artículo intercambiado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "drop", "Objeto apilado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) elemento apilado; nombre: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "drop", "Artículo recibido", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				Player.Functions.AddItem(fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
			...
			TriggerClientEvent("QBCore:Notify", src, "Este objeto ya no exite", "error")
		end
	end
end)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return TriggerClientEvent('QBCore:Notify', src, "No puedes hacer esto") end
	if dist > 2 then return TriggerClientEvent('QBCore:Notify', src, "Estás demasiado lejos para dar los artículos.") end
	local item = Player.Functions.GetItemBySlot(slot)
	if not item then TriggerClientEvent('QBCore:Notify', src, "Objeto no encontrado"); return end
	if item.name ~= name then TriggerClientEvent('QBCore:Notify', src, "Artículo incorrecto"); return end
	...
	if amount <= item.amount then
		...
		if Player.Functions.RemoveItem(item.name, amount, item.slot) then
			if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
				...
				TriggerClientEvent('QBCore:Notify', target, "Has recibido "..amount..' '..item.label.." of "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				TriggerClientEvent('QBCore:Notify', src, "Has dado" .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('qb-inventory:client:giveAnim', src)
				TriggerClientEvent('qb-inventory:client:giveAnim', target)
			else
				Player.Functions.AddItem(item.name, amount, item.slot, item.info)
				TriggerClientEvent('QBCore:Notify', src,  "El otro jugador tiene un inventario completo.", "error")
				TriggerClientEvent('QBCore:Notify', target,  "El inventario esta lleno.", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			TriggerClientEvent('QBCore:Notify', src,  "Ya no tienes", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Ya no tienes más para transferir")
	end
end)
...
QBCore.Commands.Add("resetinv", "Resetear el inventario (Administración)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID"}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "No válido", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "No has rellenado correctamente el comando", "error")
	end
end, "admin")

QBCore.Commands.Add("robar", "Robar a un usuario", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

QBCore.Commands.Add("giveitem", "Dar un item (Administración)", {{name="id", help="ID del jugador"},{name="item", help="Nombre del item"}, {name="amount", help="Cantidad"}}, true, function(source, args)
		...
		if amount > 0 then
			if itemData then
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, "Le has dado a " ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", "success")
				else
					TriggerClientEvent('QBCore:Notify', source,  "No pudo ser entregado", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source,  "El articulo no existe.", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', source,  "Cantidad invalida.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "El jugador no esta conectado", "error")
	end
end, "admin")
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body ">^j<strong>{0}</strong><br><br><strong>Nombre:</strong> {1} <br><strong>Apellido:</strong> {2} <br><strong>Fecha de nacimineto:</strong> {3} <br><strong>Licencia:</strong> {4}</div></div>',
					args = {
...
QBCore.Functions.CreateUseableItem("id_card", function(source, item)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body">^j<strong>{0}:</strong><br><br> <strong>ID:</strong> {1} <br><strong>Nombre:</strong> {2} <br><strong>Apellido:</strong> {3} <br><strong>Fecha de nacimiento:</strong> {4} <br><strong>Genero:</strong> {5} <br><strong>Nacionalidad:</strong> {6}</div></div>',
					args = {
						-- "ID"..item.info.firstname.." "..item.info.lastname.."",s
						item.info.citizenid,
						item.info.firstname,
						item.info.lastname,
						item.info.birthdate,
...
local function AddItem(source, item, amount, slot, info)
	...
	if not itemInfo and not Player.Offline then
		QBCore.Functions.Notify(source, "El articulo no existe.", 'error')
		return false
	end
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'ITEM AÑADIDO', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** tiene un artículo: [slot:' .. slot .. '], nombre: ' .. Player.PlayerData.items[slot].name .. ', cantidad: ' .. amount .. ', nuevo importe total: ' .. Player.PlayerData.items[slot].amount)
		...
		elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
			for i = 1, Config.MaxInventorySlots, 1 do
				if Player.PlayerData.items[i] == nil then
					...
					TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'ITEM AÑADIDO', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** tiene un artículo: [slot:' .. i .. '], nombre: ' .. Player.PlayerData.items[i].name .. ', cantidad: ' .. amount .. ', nuevo importe total: ' .. Player.PlayerData.items[i].amount)
	...
	elseif not Player.Offline then
		QBCore.Functions.Notify(source, "Tienes el inventario lleno.", 'error')
	end
	return false
end
